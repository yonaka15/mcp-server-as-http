...
impl McpServerProcess {
    ...
    async fn query(&mut self, request: &McpRequest) -> Result<McpResponse, String> {
        ...
        log_debug!("MCP_PROCESS", "Query #{} started - PID: {:?}", 
                  self.request_count, self.process_id);
        
        // Check if process is still alive before attempting communication
        if !self.is_process_alive().await {
            log_error!("MCP_PROCESS", "Cannot send query: MCP process has terminated");
            return Err("MCP process has terminated".to_string());
        }
        
        log_debug!("MCP_PROCESS", "Request details: {:?}", request);
        log_debug!("MCP_PROCESS", "Time since last activity: {:?}", 
        ...
        log_debug!("MCP_PROCESS", "Sending {} bytes to stdin", request_bytes.len());
        log_debug!("MCP_PROCESS", "Request content: {}", 
        ...
        match self.stdin.write_all(request_bytes).await {
            ...
            Err(e) => {
                log_error!("MCP_PROCESS", "Failed to write to stdin: {} (errno: {:?})", e, e.raw_os_error());
                return Err(format!("Failed to write to MCP stdin: {} (errno: {:?})", e, e.raw_os_error()));
            }
        }
        ...
        match self.stdin.flush().await {
            ...
            Err(e) => {
                log_error!("MCP_PROCESS", "Failed to flush stdin: {} (errno: {:?})", e, e.raw_os_error());
                return Err(format!("Failed to flush MCP stdin: {} (errno: {:?})", e, e.raw_os_error()));
            }
        }

        log_debug!("MCP_PROCESS", "Request sent, waiting for response (timeout: {}s)", 
                  self.config.response_timeout_secs);

        // Read response with enhanced timeout and error tracking
        let response_result = timeout(Duration::from_secs(self.config.response_timeout_secs), async {
            ...
            match self.stdout.read_line(&mut response_line).await {
                ...
                Ok(bytes_read) => {
                    log_debug!("MCP_PROCESS", "Read {} bytes in {:?}", 
                              bytes_read, read_start.elapsed());
                    ...
                    if response.is_empty() {
                        ...
                        log_debug!("MCP_PROCESS", "Response content: {}", 
                ...
                Err(e) => {
                    log_error!("MCP_PROCESS", "Failed to read response: {} (errno: {:?})", 
                              e, e.raw_os_error());
                    Err(format!("Failed to read response: {} (errno: {:?})", e, e.raw_os_error()))
                }
            }
        }).await;
        ...
        match response_result {
            Ok(result) => {
                log_info!("MCP_PROCESS", "Query #{} completed successfully in {:?}", 
                         self.request_count, query_start.elapsed());
                result
            }
            Err(_) => {
                log_error!("MCP_PROCESS", "Query #{} timed out after {:?}", 
                          self.request_count, query_start.elapsed());
...
async fn setup_mcp_server(
    ...
    if !server_config.supported_server_types.contains(&config.server_type) {
        log_error!("SETUP", "Unsupported server type: {} (supported: {:?})", 
                  config.server_type, server_config.supported_server_types);
        return Err(format!("Unsupported server type: {} (supported: {:?})", 
                          config.server_type, server_config.supported_server_types));
    }
    ...
    if !tokio::fs::metadata(&server_dir).await.is_ok() {
        ...
        let output = Command::new("git")
            .args(&["clone", &format!("https://github.com/{}", repo), &server_dir])
            .output()
            .await
            .map_err(|e| {
        ...
        log_info!("SETUP", "Repository cloned successfully in {:?}", clone_start.elapsed());
        need_install = true;
    } else {
        log_debug!("SETUP", "Directory {} already exists, skipping clone", server_dir);
    }
    ...
    if !tokio::fs::metadata(&entrypoint_path).await.is_ok() {
        log_warn!("SETUP", "Entrypoint not found: {}, will run install command", entrypoint_path);
        need_install = true;
    }
        ...
        if let Some(install_cmd) = &config.install_command {
            ...
            let output = if install_cmd.contains("&&") || install_cmd.contains("||") {
                ...
                Command::new("sh")
                    ...
                    .await
                    .map_err(|e| {
                        log_error!("SETUP", "Failed to execute install command via shell: {}", e);
            ...
            log_info!("SETUP", "Dependencies installed in {:?}", install_start.elapsed());
        } else {
            log_warn!("SETUP", "Entrypoint missing but no install command specified");
        }
    }
...
fn build_command(
    ...
    if !server_config.supported_languages.contains(&config.language) {
        return Err(format!("Unsupported language: {} (supported: {:?})", 
    ...
    match config.language.as_str() {
        ...
        lang => Err(format!("Language '{}' is supported but not implemented", lang))
    }
}
...
async fn start_mcp_server(
    ...
    log_info!("MCP_SERVER", "Starting MCP server setup for '{}'", server_name);
    ...
    let mcp_server_config = configs.get(server_name)
        .ok_or_else(|| format!("Server '{}' not found in config", server_name))?;

    // Setup server
    setup_mcp_server(server_name, mcp_server_config, server_config).await
    ...
    let (stdin, stdout, stderr) = {
        ...
        if let Some(ref mut child) = child_guard.as_mut() {
            match child.try_wait() {
                Ok(Some(status)) => {
                    log_error!("MCP_SERVER", "Process exited immediately with status: {}", status);
                    return Err(format!("Process exited immediately with status: {}", status).into());
                }
                Ok(None) => {
                    log_info!("MCP_SERVER", "Process is running healthy - PID: {:?}, startup time: {:?}", 
                             process_id, process_start.elapsed());
                }
                Err(e) => {
    ...
    tokio::spawn(async move {
        ...
        log_debug!("STDERR_MONITOR", "Starting stderr monitoring for {}", server_name_clone);
        
        while let Ok(n) = reader.read_line(&mut line).await {
            if n == 0 { 
                log_warn!("STDERR_MONITOR", "Process {} terminated (stderr closed)", server_name_clone);
                ...
                if let Some(child) = child_guard.as_mut() {
                    match child.try_wait() {
                        Ok(Some(status)) => {
                            log_error!("STDERR_MONITOR", "Process {} exited with status: {}", 
                                      server_name_clone, status);
                        }
                        Ok(None) => {
                            log_warn!("STDERR_MONITOR", "Process {} stderr closed but process still running", 
                                     server_name_clone);
                        }
                        Err(e) => {
            ...
            if !trimmed_line.is_empty() {
                log_debug!("STDERR_MONITOR", "[{}:{}] {}", 
                          server_name_clone, line_count, trimmed_line);
            }
            
            line.clear();
        }
        
        log_info!("STDERR_MONITOR", "Stderr monitoring ended for {} after {} lines", 
                 server_name_clone, line_count);
    });

    // Wait for process initialization with progress logging
    log_debug!("MCP_SERVER", "Waiting for process initialization ({}s)", 
              server_config.process_init_wait_secs);
    tokio::time::sleep(Duration::from_secs(server_config.process_init_wait_secs)).await;

    let now = Instant::now();
    log_info!("MCP_SERVER", "MCP server '{}' started successfully - Total setup time: {:?}", 
             server_name, process_start.elapsed());
...
async fn handle_request(
    ...
    log_debug!("HTTP_HANDLER", "Acquired process lock in {:?}", lock_start.elapsed());
    ...
    match process.query(&payload).await {
        Ok(response) => {
            log_info!("HTTP_HANDLER", "Request completed successfully in {:?}", 
                     request_start.elapsed());
            log_debug!("HTTP_HANDLER", "Response size: {} chars", response.result.len());
            Ok(AxumJson(response))
        }
        Err(e) => {
            log_error!("HTTP_HANDLER", "Request failed after {:?}: {}", 
                      request_start.elapsed(), e);
            log_error!("HTTP_HANDLER", "Process stats at failure: {}", process.get_stats());
...
async fn main() {
    ...
    let mcp_process = match start_mcp_server(&config_file, &server_name, &server_config).await {
        Ok(process) => {
            log_info!("MAIN", "MCP server initialized in {:?}", mcp_start.elapsed());
            Arc::new(Mutex::new(process))
        }
        Err(e) => {
            log_error!("MAIN", "Failed to start MCP server after {:?}: {}", 
                      mcp_start.elapsed(), e);
            return;
        }
    };
